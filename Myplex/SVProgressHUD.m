...
- (void)overlayViewDidReceiveTouchEvent:(id)sender forEvent:(UIEvent *)event {
    [[NSNotificationCenter defaultCenter] postNotificationNameOnMainThread:SVProgressHUDDidReceiveTouchEventNotification object:event];
}
...
- (void)showProgress:(float)progress status:(NSString*)string maskType:(SVProgressHUDMaskType)hudMaskType {
    ...
    if(self.alpha != 1) {
        ...
        [[NSNotificationCenter defaultCenter] postNotificationNameOnMainThread:SVProgressHUDWillAppearNotification
                                                            object:nil
                                                          userInfo:userInfo];
        ...
        [UIView animateWithDuration:0.15
                         ...
                         completion:^(BOOL finished){
                             [[NSNotificationCenter defaultCenter] postNotificationNameOnMainThread:SVProgressHUDDidAppearNotification
                                                                                 object:nil
                                                                               userInfo:userInfo];
...
- (void)dismiss {
    NSDictionary *userInfo = [self notificationUserInfo];
    [[NSNotificationCenter defaultCenter] postNotificationNameOnMainThread:SVProgressHUDWillDisappearNotification
                                                        object:nil
                                                      userInfo:userInfo];
    ...
    [UIView animateWithDuration:0.15
                     ...
                     completion:^(BOOL finished){
                         if(self.alpha == 0 || self.hudView.alpha == 0) {
                             ...
                             [[NSNotificationCenter defaultCenter] postNotificationNameOnMainThread:SVProgressHUDDidDisappearNotification
                                                                                 object:nil
                                                                               userInfo:userInfo];
...
- (UIFont *)hudFont {
    ...
    return [UIFont fontWithName:@"MuseoSansRounded-700" size:14.0];//[UIFont boldSystemFontOfSize:16];
}
